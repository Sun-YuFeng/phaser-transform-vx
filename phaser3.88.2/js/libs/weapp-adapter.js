...
/***/ (function(module, exports, __webpack_require__) {
	...
	if (!GameGlobal.__isAdapterInjected) {
	  ...
	  GameGlobal.wxGetWindowInfo = function wxGetWindowInfo() {
	  ...
	  GameGlobal.wxSetupCanvas = function wxSetupCanvas(targetCanvas) {
	    var c = targetCanvas || window.canvas;
	    if (!c) return null;
	    var info = GameGlobal.wxGetWindowInfo();
	    var dpr = info.pixelRatio || 1;
	    var w = info.windowWidth || info.screenWidth || 375;
	    var h = info.windowHeight || info.screenHeight || 667;
	    c.width = w * dpr;
	    c.height = h * dpr;
	    return c;
	  };
...
/***/ (function(module, exports) {
	...
	function createFallbackPerformance() {
	...
	if (sysInfo.platform !== 'devtools' && wx.getPerformance) {
	  try {
	    ...
	    if (wxPerf && typeof wxPerf.now === 'function') {
	      ...
	      performance = createFallbackPerformance();
	    }
	  } catch (e) {
	    performance = createFallbackPerformance();
	  }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var document = {
	  ...
	  createElement: function createElement(tagName) {
	    ...
	    var el = new _HTMLElement2.default(tagName);
	    if (tagName === 'style') {
	  ...
	  createTextNode: function createTextNode(text) {
	    var node = new _HTMLElement2.default('');
	    node.nodeType = 3;
	    node.nodeValue = text;
	  ...
	  elementFromPoint: function elementFromPoint(x, y) {
	    // 微信试玩全屏单 canvas；真机 touch 坐标与 innerWidth 可能不一致，勿做边界判断
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Audio = function (_HTMLAudioElement) {
	  ...
	  _createClass(Audio, [{
	    ...
	    key: 'play',
	    value: function play() {
	      var ctx = _innerAudioContext.get(this);
	      try {
	        ctx.play();
	      } catch (err) {
	        return Promise.reject(err);
	      }
	      // H5 代码普遍 s.play().catch(...)；微信 InnerAudioContext.play() 无返回值
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function normalizeTouchPoint(t) {
	  ...
	  return {
	    ...
	    clientX: t.clientX != null ? t.clientX : x,
	    clientY: t.clientY != null ? t.clientY : y,
	    pageX: t.pageX != null ? t.pageX : x,
	    pageY: t.pageY != null ? t.pageY : y,
	    x: x,
	    y: y,
	    // Phaser 3.88：Pointer.touchstart 用 touch.target 作 downElement，须 === canvas 才发 POINTER_DOWN
	    target: t.target || window.canvas
	  };
	}

	function normalizeTouchList(list) {
	  if (!list || !list.length) return [];
	  var out = [];
	  for (var i = 0; i < list.length; i++) {
	    out.push(normalizeTouchPoint(list[i]));
	  }
	  return out;
	}
	...
	function touchEventHandlerFactory(type) {
	  return function (event) {
	    ...
	    touchEvent.touches = normalizeTouchList(event.touches);
	    touchEvent.targetTouches = normalizeTouchList(event.touches);
	    touchEvent.changedTouches = normalizeTouchList(event.changedTouches);
