	...
	var Node = function (_EventTarget) {
	  ...
	  _createClass(Node, [{
	    ...
	    value: function appendChild(node) {
	      var attach = typeof GameGlobal !== 'undefined' && GameGlobal.setWxParent;
	      if (node instanceof Node) {
	        var prev = attach ? GameGlobal.getWxParent(node) : node.parentNode;
	        if (prev && prev !== this && prev.removeChild) {
	          prev.removeChild(node);
	        }
	        if (attach) GameGlobal.setWxParent(node, this);
	        else node.parentNode = this;
	        this.childNodes.push(node);
	      } else if (node && typeof node === 'object') {
	        var prevNode = attach ? GameGlobal.getWxParent(node) : node.parentNode;
	        if (prevNode && prevNode !== this && prevNode.removeChild) {
	          prevNode.removeChild(node);
	        }
	        if (attach) GameGlobal.setWxParent(node, this);
	        else {
	          try {
	    ...
	    value: function removeChild(node) {
	      ...
	      if (index > -1) {
	        if (typeof GameGlobal !== 'undefined' && GameGlobal.setWxParent) {
	          GameGlobal.setWxParent(node, null);
	        } else {
	          try {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var document = {
	  ...
	  getElementById: function getElementById(id) {
	    ...
	    var reg = typeof GameGlobal !== 'undefined' && GameGlobal.__wxElementById;
	    if (reg && reg[id]) {
	      return reg[id];
	    }
	    return null;
	  ...
	  querySelector: function querySelector(query) {
	    if (query === 'head') {
	      ...
	      return window.canvas;
	    } else if (query && query.charAt(0) === '#') {
	      var reg = typeof GameGlobal !== 'undefined' && GameGlobal.__wxElementById;
	      var byId = reg && reg[query.slice(1)];
	      if (byId) return byId;
	    }
	    return null;
	  },
